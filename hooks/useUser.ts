		...
		try {
			...
			if (response.ok) {
				...
				setBillingInfo({
					...
					premium_started_at: data.profile?.is_premium
	...
	const fetchAuthorProfiles = useCallback(async (missingIds: string[]) => {
		...
		try {
			const { data } = await supabase
				.from("profiles")
				.select(
					"user_id,username,avatar_url,cover_url,display_name,bio,is_premium",
				)
			...
			setError(
				err instanceof Error ? err.message : "Failed to fetch author profiles",
			);
		} finally {
			setLoading(false);
		}
