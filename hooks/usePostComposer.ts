...
export function usePostComposer(authorId: string) {
	...
	const uploadOne = useCallback(
		async (postId: string, file: FileLike) => {
			const ext = (file.type.split("/")[1] || "jpg").replace("jpeg", "jpg") as
				| "jpg"
				| "png"
				| "webp";
	...
	const create = useCallback(
		async (caption: string | undefined, files: FileLike[]) => {
			...
			try {
				...
				console.log(`[CLIENT] Calling createPost with data:`, {
					author_id: authorId,
					text: caption,
					images: metas,
				});
	...
	const edit = useCallback(
		async (post_id: string, caption?: string, images?: ImageMetaRow[]) => {
			setBusy(true);
			setError(null);
			try {
