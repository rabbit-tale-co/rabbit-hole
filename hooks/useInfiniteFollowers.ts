...
import { getFollowersPage } from "@/app/actions/follow";
...
export function useInfiniteFollowers(
  targetUserId: string,
  initial?: { items: FollowerItem[]; nextCursor: string | null },
  pageSize = 24
) {
  const [pages, setPages] = useState<{ items: FollowerItem[]; nextCursor: string | null }[]>(
    initial ? [initial] : []
  );
  ...
  const loadMore = useCallback(async () => {
    ...
    const res = await getFollowersPage(targetUserId, cursorParam, pageSize);
    if ("error" in res && res.error) {
      setError(res.error);
    } else if ("items" in res) {
      setPages(prev => [...prev, { items: res.items ?? [], nextCursor: res.nextCursor ?? null } as { items: FollowerItem[]; nextCursor: string | null }]);
