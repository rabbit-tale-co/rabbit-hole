...
export function useFollow(
	...
	initialStats?: { isFollowing: boolean; followers: number; following: number },
) {
	const { user } = useAuth();
	const [isFollowing, setIsFollowing] = useState(
		initialStats?.isFollowing ?? false,
	);
	...
	const [loading, setLoading] = useState(
		Boolean(targetUserId && !initialStats),
	);
		...
		startTransition(async () => {
			...
			if (res.ok) {
				...
				if (res.error === "UNAUTHORIZED") {
					...
					toast.error(
						`Failed to update follow status: ${res.error || "Unknown error"}`,
					);
				}
			}
		});
