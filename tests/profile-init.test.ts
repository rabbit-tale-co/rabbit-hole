...
describe("Profile Init API", () => {
  ...
  describe("Username Validation", () => {
    it("should reject usernames with bannable words", async () => {
      ...
      const result = await containsBannableWords("inappropriate_username", mockGetConfigFromEnv());
    ...
    it("should allow clean usernames", async () => {
      ...
      const result = await containsBannableWords("clean_username", mockGetConfigFromEnv());
    ...
    it("should allow mild profanity usernames for 16+", async () => {
      ...
      // Test the bannable words function directly
      const result = await containsBannableWords("fuck_this_username", mockGetConfigFromEnv());
  ...
  describe("Configuration", () => {
    ...
    it("should have obscenity config with age rating", () => {
      ...
      expect(['all', 'teen', 'mature', 'adult']).toContain(config.obscenityConfig?.ageRating);
    });
  });
});
