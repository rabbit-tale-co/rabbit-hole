 ...
 */
export async function ensureCustomerOnProfile(
	userId: string,
	...
	try {
		...
		if (!profile.stripe_customer_id) {
			...
			if (updateError) {
				throw new Error(
					`Failed to update profile with customer ID: ${updateError.message}`,
				);
			}

			console.log(
				`✅ Linked Stripe customer ${stripeCustomerId} to user ${userId}`,
			);
		} else {
			console.log(
				`ℹ️  User ${userId} already has customer ID: ${profile.stripe_customer_id}`,
			);
		}
	} catch (error) {
...
export async function upsertSubscription(
	...
	try {
		console.log(
			`📝 Upserting subscription ${subscription.id} for customer ${subscription.customer}`,
		);
		...
		const { error: recomputeError } = await supabaseAdmin.rpc(
			"recompute_is_premium",
			{ p_user_id: userId },
		);

		if (recomputeError) {
			console.warn(
				`⚠️  Failed to recompute premium status: ${recomputeError.message}`,
			);
		} else {
