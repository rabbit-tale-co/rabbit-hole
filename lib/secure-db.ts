 ...
 */
export async function getSecureUsersList(limit: number = 20, offset: number = 0) {
  try {
    const { data, error } = await supabaseAdmin
      .schema('social_art')
      .from('profiles')
 ...
 */
export async function getSecureUserPosts(userId: string, limit: number = 20, offset: number = 0) {
  try {
    const { data, error } = await supabaseAdmin
      .schema('social_art')
      .from('posts')
 ...
 */
export function logSecureError(operation: string, error: unknown, userId?: string) {
