export function buildPublicUrl(path: string): string {
	...
	try {
		if (/^https?:\/\//.test(path)) {
			...
			const looksLikeKey = /^\/(posts|avatar|cover|social|media)\//.test(
				u.pathname,
			);
	...
	const endpointRaw = (
		...
		process.env.SOCIAL_S3_ENDPOINT ||
		""
	).replace(/\/$/, "");
	const bucket =
		process.env.NEXT_PUBLIC_S3_BUCKET || process.env.SOCIAL_S3_BUCKET || "";
	if (endpointRaw && bucket && key) {
		const base = /^https?:\/\//.test(endpointRaw)
			? endpointRaw
			: `https://${endpointRaw}`;
