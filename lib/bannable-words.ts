...
async function fetchBannableWords(): Promise<Set<string>> {
	...
	const customWords =
		process.env.BANNABLE_WORDS?.split(",").map((w) => w.trim().toLowerCase()) ||
		[];
...
export async function containsBannableWords(
	...
	if (config.useObscenity && config.obscenityConfig) {
		try {
			...
			console.warn(
				"Obscenity detection failed, falling back to traditional method:",
				error,
			);
		}
	}
	...
	const ageAppropriateWords =
		config.obscenityConfig?.ageRating === "mature" ||
...
export async function validateText(
	...
	if (result.hasBannableWords) {
		console.log(
			`[BANNABLE] Found bannable words in "${text}":`,
			result.foundWords,
		);
		return {
			isValid: false,
...
export async function sanitizeText(
	...
	if (config.useObscenity && config.obscenityConfig) {
		try {
			...
			console.warn(
				"Obscenity censoring failed, falling back to traditional method:",
				error,
			);
		}
	}
...
export function getConfigFromEnv(): BannableWordsConfig {
	return {
		...
		customWords:
			process.env.BANNABLE_WORDS_CUSTOM?.split(",").map((w) => w.trim()) || [],
		whitelist:
			process.env.BANNABLE_WORDS_WHITELIST?.split(",").map((w) => w.trim()) ||
			[],
