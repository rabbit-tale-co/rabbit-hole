...
function DialogToast({
  ...
  React.useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      ).triggerToastShake = triggerShake;
    }
    return () => {
      if (typeof window !== "undefined") {
        delete (window as Window & { triggerToastShake?: () => void })
          .triggerToastShake;
      }
    };
  }, [triggerShake]);
      ...
      {show && (
        <motion.div
          ...
          className={cn(
            ...
            "w-full min-w-[20rem] max-w-4xl px-4",
            "sm:min-w-[32rem]",
          )}
        ...
        >
          <div className="flex sm:h-12 items-center justify-between rounded-full border bg-white p-1 pl-3 shadow-lg">
            <span className="text-xs sm:text-sm font-medium">{message}</span>
            <div className="ml-2 sm:ml-4 flex gap-1 sm:gap-2">
              <Button
                onClick={onReset}
                variant="ghost"
              >
...
function isFromToast(e: Event) {
  ...
  return path
    ? path.some((n) => n instanceof Node && toastEl.contains(n as Node))
    : false;
}
