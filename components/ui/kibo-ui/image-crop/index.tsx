...
export const ImageCropContent = ({
  ...
  return (
    <ReactCrop
      className={cn('max-h-[50vh] max-w-[80vw] sm:max-w-[60vw] overflow-hidden', className)}
      crop={crop}
      ...
      {imgSrc && dims && (
        <Image
          ...
          onLoadingComplete={(el) => {
            try { (imgRef as unknown as React.MutableRefObject<HTMLImageElement | null>).current = el; } catch { }
            try { onImageLoad({ currentTarget: el } as unknown as SyntheticEvent<HTMLImageElement>); } catch { }
          }}
        />
      )}
    </ReactCrop>
