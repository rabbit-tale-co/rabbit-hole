...
export function UserAvatar({
  ...
  // Get color styles for fallback - different shades for rabbit holes
  const avatarBgStyle = accentHex
    ? variant === "rabbit-hole"
      ? getStyleFromHex(accentHex, "backgroundColor")
      : getStyleFromHexShade(accentHex, "200", "backgroundColor")
    : getAccentColorStyle(currentAccentColor, variant === "rabbit-hole" ? 500 : 200, "backgroundColor");
  const avatarForegroundStyle = accentHex
    ? variant === "rabbit-hole"
      ? getStyleFromHexShade(accentHex, "950", "color")
      : getStyleFromHexShade(accentHex, "950", "color")
    : getAccentColorStyle(currentAccentColor, variant === "rabbit-hole" ? 50 : 950, "color");
