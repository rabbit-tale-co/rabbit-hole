...
import { OutlinePlus, SolidLogo, OutlineChevronRight } from "@/components/icons/Icons"
...
export default function Header() {
  ...
  const [rabbitHoles, setRabbitHoles] = useState<RabbitHole[]>([
    ...
    { id: "art", name: "Art & Creative", color: "bg-indigo-500", memberCount: 345 },
  ])
    ...
    <>
      {navigationLinks.map((link) => {
        ...
        return (
          ...
          >
            <Link href={link.href}>{link.label}{link.icon}</Link>
          </Button>
        )
      })}
    </>
  ...
  const MobileBottomNav = () => {
    ...
    useEffect(() => {
      ...
      const apply = () => {
        ...
        try {
          document.documentElement.style.setProperty("--mobile-bottom-nav-height", `${h}px`)
        } catch { }
      }
      apply()
        ...
        <div className="flex items-center justify-between h-16 gap-4">
          <div className="flex items-center gap-2">
            ...
            <div className="hidden lg:block p-1 rounded-full bg-background">
              <DropdownMenu open={isDesktopExpanded} onOpenChange={setIsDesktopExpanded}>
                <DropdownMenuTrigger asChild>
                  ...
                  >
                    Rabbit Holes
                    <OutlineChevronRight size={16} className={cn("transition-transform duration-150", isDesktopExpanded ? "rotate-90" : "")} />
                  </Button>
                </DropdownMenuTrigger>
                  ...
                  <div className="max-h-60 overflow-y-auto">
                    <div className="grid grid-cols-1 gap-2">
                      {rabbitHoles.map((hole) => {
                        const isActive = pathname === `/rabbit-hole/${hole.id}` || (pathname === "/" && hole.id === "general")

                        return (
                          <DropdownMenuItem key={hole.id} asChild>
                            <Link href={hole.id === "general" ? "/" : `/rabbit-hole/${hole.id}`} className="flex items-center gap-3 p-2">
                              <div className={cn(
                                "size-2 rounded-full ring-1 ring-border",
                                hole.color
                              )} />
                              <div className="flex flex-col items-start">
                                <span className={cn("text-sm font-medium", isActive && "text-primary")}>{hole.name}</span>
                                <span className={cn(
                                  "text-xs",
                                  isActive ? "text-primary/70" : "text-muted-foreground"
                                )}>{hole.memberCount} members</span>
                              </div>
                            </Link>
                          </DropdownMenuItem>
                        )
            ...
            <div className="block lg:hidden p-1 rounded-full bg-background">
              <Drawer open={isMobileExpanded} onOpenChange={setIsMobileExpanded}>
                <DrawerTrigger asChild>
                  ...
                  >
                    Rabbit Holes
                    <OutlineChevronRight size={16} className={cn("ml-2 transition-transform duration-150", isMobileExpanded ? "rotate-90" : "")} />
                  </Button>
                </DrawerTrigger>
                      ...
                      <div className="space-y-2">
                        {rabbitHoles.map((hole) => {
                          ...
                          return (
                            ...
                            >
                              <Link href={hole.id === "general" ? "/" : `/rabbit-hole/${hole.id}`} className="flex items-center gap-3 w-full">
                                <div className={cn(
                                  "size-2 rounded-full flex-shrink-0 ring-1 ring-border",
                                  hole.color
                                )} />
                                <div className="flex flex-col items-start text-left min-w-0">
                                  <span className="font-medium truncate">{hole.name}</span>
                                  <span className={cn(
                                    "text-xs truncate",
