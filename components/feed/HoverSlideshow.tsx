...
export function HoverSlideshow({
  ...
  const { hovered, setHovered, currentIndex, progress } = useHoverSlideshow(imgs);
    ...
    >
      {imgs.map((src, i) => (
        <Image
          ...
          fill
          sizes={`${widthPx}px`}
          className={`object-cover transition-opacity duration-300 ${i === currentIndex ? "opacity-100" : "opacity-0"
            }`}
          unoptimized
          quality={100}
          priority={false}
      ...
      {imgs.length > 1 && (
        <div className="absolute bottom-1 left-0 right-0 z-20 flex gap-1 px-6">
          {imgs.map((_, i) => (
            <Progress
              key={i}
              value={hovered ? (i < currentIndex ? 100 : i === currentIndex ? progress : 0) : 0}
