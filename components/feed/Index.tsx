...
export default function Feed({ initial, debugLoading, authorId, isOwnProfile }: { initial?: Parameters<typeof useInfiniteFeed>[0]; debugLoading?: boolean; authorId?: string; isOwnProfile?: boolean }) {
  // data
      ...
      <div className="relative" style={{ height: containerHeight }}>
        {placed.map((p) => {
          ...
          return (
            <article
              key={p.tile.id}
              className="absolute bg-white ring-1 ring-[--border] rounded-2xl overflow-hidden"
              style={{ top, left, width, height }}
            >
              <Image
                src={p.tile.cover.path /* if this is a storage key, swap to your publicURL helper */}
