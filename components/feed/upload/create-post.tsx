...
export function CreateMediaPost({
  ...
  useEffect(
    () => () =>
      items.forEach((i) => {
        if (i.preview && i.preview.startsWith("blob:"))
  ...
  const removeById = useCallback(
    (id: string) => {
      setItems((prev) => {
        ...
        if (rem?.preview && rem.preview.startsWith("blob:"))
          URL.revokeObjectURL(rem.preview);
        try {
          uploadActions.removeFile(id);
        } catch { }
  ...
  const uploadOne = useCallback(
    async (it: Item, postId: string, onProgress: (pct: number) => void) => {
      ...
      if (it.kind === "video" || blob.size > 4 * 1024 * 1024) {
        ...
        await new Promise<void>(async (resolve, reject) => {
          try {
            ...
            onProgress(1);
            const res = await fetch(`${MEDIA_API}/social/v1/post/upload`, {
              method: "POST",
              body: fd,
            });
            if (!res.ok) {
            ...
            try {
              patchItem(it.id, {
                width: serverMeta.width,
                height: serverMeta.height,
                serverPath: serverMeta.path,
          ...
          const built = {
            ...
            mime: (json.mime ||
              (it.kind === "gif" || it.kind === "video"
                ? "video/webm"
                : "image/webp")) as NonNullable<Item["mime"]>,
            is_cover: Boolean(it.isCover),
          } as Meta;
      ...
      <div className="flex relative flex-col gap-2 p-3">
        <div className="flex items-center justify-between gap-2 px-1">
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            ...
            {profile === null && (
              <span className="text-orange-500">(Loading profile...)</span>
            )}
          </div>
        ...
        {items.length === 0 ? (
          <div
            ...
            onDrop={(e) => {
              e.preventDefault();
              setOverDrop(false);
              uploadActions.addFiles(e.dataTransfer.files);
            }}
            className={[
          ...
          >
            ...
            <div className="text-sm text-muted-foreground hidden lg:block">
              Drag & drop images or a video, paste (Ctrl/Cmd+V), or{" "}
              <span className="underline underline-offset-2">browse</span>
            </div>
            <div className="mt-1 text-[11px] text-muted-foreground/80">
              {MAX_IMAGES} images max • {MAX_VIDEOS} video max •{" "}
              {actualFileSizeMbMax}MB each
            </div>
          </div>
        ) : (
          <>
            ...
            <div className="flex items-center justify-between py-2">
              <div className="text-[12px] text-muted-foreground">
                {countImages}/{MAX_IMAGES} images • {countVideos}/{MAX_VIDEOS}{" "}
                videos
              </div>
                ...
                >
                  ...
                  >
                    ...
                    <SortableItemHandle className="absolute top-1.5 left-1.5">
                      <div
                        className="inline-flex size-6 items-center justify-center rounded-full bg-background/80
            ...
            {overallProgress > 0 && overallProgress < 100 && (
              <div className="mt-2">
                <div className="h-1 w-full bg-black/10 rounded">
                  <div
                    className="h-1 bg-primary/80 rounded"
              ...
              <input
                ...
                onChange={(e) =>
                  patchItem(selected.id, {
                    alt: e.target.value.slice(0, MAX_ALT),
                  })
                }
