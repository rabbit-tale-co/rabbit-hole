...
export default function ProfileEmptyGallery({
  ...
  const CreatePostDialog = React.useMemo(
    () =>
      dynamic(
        () =>
          import("@/components/feed/upload/create-post").then(
            (m) => m.CreatePost,
          ),
        { ssr: false },
      ),
  ...
  return (
    <div className="mx-auto mt-10 max-w-6xl px-3 pb-20">
        ...
        <CardContent className="p-0">
          ...
          <div className="relative grid gap-6 p-6 sm:p-10 lg:grid-cols-[1.2fr_.8fr]">
              ...
              >
                {isOwnProfile ? (
                  <>
                    <Button
                      onClick={
                        onCreate ? onCreate : () => setOpenComposer(true)
                      }
                      className="gap-2"
                    >
            ...
            <div className="relative">
              ...
              >
                <Badge
                  icon={<OutlineImage className="size-3" />}
                  label="WIP shots"
                />
                <Badge
                  icon={<OutlineVideo className="siz-3" />}
                  label="Timelapse"
                />
              </motion.div>
            </div>
      ...
      <Dialog
        open={isOwnProfile && openComposer}
  ...
  return (
    <Center>
      <div className="mx-auto max-w-4xl px-3 pb-20 text-center">
        ...
        >
          <div className="mb-6 size-24 items-center justify-center flex rounded-full bg-muted/50">
            <OutlineImage size={48} className="text-muted-foreground" />
          </div>
...
function FollowingEmptyFeed() {
  return (
    <div className="mt-8 rounded-2xl bg-white ring-1 ring-[--border] p-10 text-center">
      <div className="mx-auto mb-4 flex size-16 items-center justify-center rounded-full bg-muted">
        <OutlineUser size={32} className="text-muted-foreground" />
      </div>
      <h3 className="text-lg font-semibold">No posts yet</h3>
      <p className="mt-2 text-sm text-muted-foreground">
        When people you follow post something, it'll show up here.
      </p>
      <div className="mt-6">
        <Link
          href="/explore"
...
function RabbitHoleEmptyFeed({ message, description }: { message?: string; description?: string }) {
  return (
    <div className="mt-8 rounded-2xl bg-white ring-1 ring-[--border] p-10 text-center">
      <div className="mx-auto mb-4 flex size-16 items-center justify-center rounded-full bg-muted">
        <svg
      ...
      <h3 className="text-lg font-semibold">{message || "No posts yet"}</h3>
      <p className="mt-2 text-sm text-muted-foreground">
        {description || "Be the first to post in this rabbit hole!"}
      </p>
    </div>
  );
}
...
export function EmptyState({
  ...
  variant: "profile" | "home" | "following" | "rabbit-hole";
  isOwnProfile?: boolean;
  message?: string;
  description?: string;
  onCreate?: () => void;
  ...
  if (variant === "following") {
    return <FollowingEmptyFeed />;
  }

  if (variant === "rabbit-hole") {
    return <RabbitHoleEmptyFeed message={message} description={description} />;
  }
  return (
    <ProfileEmptyGallery
