...
const statusChip = (s: string) => {
	...
	switch (s) {
		...
		case "refunded":
			return (
				<span className={`${base} bg-blue-100 text-blue-700`}>REFUNDED</span>
			);
		case "void":
			return (
				<span className={`${base} bg-neutral-100 text-neutral-700`}>VOID</span>
			);
		default:
...
const subscriptionStatusChip = (status: string, isCanceling: boolean) => {
	...
	switch (status) {
		...
		case "trialing":
			return (
				<Badge className={"bg-blue-100 text-blue-700 !text-xs"}>TRIALING</Badge>
			);
		case "canceled":
		case "cancelled":
			return (
				<Badge className={"bg-red-100 text-red-700 !text-xs"}>CANCELED</Badge>
			);
		case "past_due":
			return (
...
export default function BillingSettings() {
	...
	const fetchBillingData = useCallback(async () => {
		...
		try {
			...
			if (res.ok) {
				...
				toast.error(
					data.error || `Failed to fetch billing data (${res.status})`,
				);
			}
		} catch (e) {
	...
	const triggerCancelDialog = () => {
		...
		window.dispatchEvent(
			new CustomEvent("settings:showCancelDialog", {
				...
				<div className="space-y-2 [&>*>span]:text-sm">
					{loading ? (
						...
						<>
							...
							{billingData?.currentSubscription?.cancel_at_period_end &&
								billingData?.currentSubscription?.current_period_end && (
									<>
										<div className="flex items-center justify-between py-1.5">
																	...
																	<div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
																		<div className="space-y-2">
																			<div className="flex justify-between">
																				...
																				<span
																					className="font-mono text-xs text-right max-w-[200px] truncate"
																					title={item.id}
																				>
																					{item.id}
																				</span>
