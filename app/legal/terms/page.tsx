...
import { useActiveSection, useSmoothScroll } from "@/hooks/useUI";
import {
  ADDRESS,
  COMPANY,
  CONTACT_EMAIL,
...
function RenderList({
  ...
  items: (string | { text: string; items: ListItem[] })[];
}) {
  return (
    <TypographyList>
      {items.map((item, idx) => {
...
function renderInline(
  ...
  const parseItalic = (input: string): React.ReactNode => {
    ...
    while ((match = regex.exec(input)) !== null) {
      if (match.index > lastIndex)
        elements.push(
          <span key={`it-text-${elements.length}`}>
      ...
      if (match.index > lastIndex)
        elements.push(
          <span key={`text-${elements.length}`}>
            {parseItalic(input.slice(lastIndex, match.index))}
          </span>,
        );
      elements.push(
        <strong key={`bold-${elements.length}`}>{match[1]}</strong>,
      );
      lastIndex = regex.lastIndex;
    }
    if (lastIndex < input.length)
      elements.push(
        <span key={`text-${elements.length}`}>
          {parseItalic(input.slice(lastIndex))}
        </span>,
      );
    return <>{elements}</>;
  };
  ...
  const parseLinks = (input: string): React.ReactNode => {
      ...
      if (match.index > lastIndex)
        parts.push(
          <span key={`t-${parts.length}`}>
            {parseBold(input.slice(lastIndex, match.index))}
          </span>,
        );
      const label = match[1]
        .replaceAll("{SITE}", SITE)
    ...
    if (lastIndex < input.length)
      parts.push(
        <span key={`t-${parts.length}`}>
          {parseBold(input.slice(lastIndex))}
        </span>,
      );
    return <>{parts}</>;
  };
