...
export async function getBatchFollowStats(
	...
	if (followersData) {
		...
		for (const rel of followersData) {
			followersCount.set(
				rel.following_id,
				(followersCount.get(rel.following_id) || 0) + 1,
			);
		}
	...
	if (followingData) {
		...
		for (const rel of followingData) {
			followingCount.set(
				rel.follower_id,
				(followingCount.get(rel.follower_id) || 0) + 1,
			);
		}
...
export async function toggleFollow(
	targetUserId: string,
	currentUserId: string,
): Promise<FollowStats & { ok: boolean; error?: string }> {
	// Log JWT usage for follow operations
	console.log(
		`[JWT] Follow toggle requested: ${currentUserId} -> ${targetUserId}`,
	);
...
> {
	...
	if (cursor) {
		try {
			const [timestamp] = Buffer.from(cursor, "base64")
				.toString("utf8")
				.split("|");
	...
	const nextCursor =
		items.length === limit && followData?.length === limit
			? Buffer.from(
					`${followData[followData.length - 1].created_at}|${items[items.length - 1].username}`,
				).toString("base64")
			: null;
