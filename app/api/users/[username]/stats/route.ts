...
export async function GET(
	...
	if (!parsed.success)
		return Response.json({ error: "bad username" }, { status: 400 });
	...
	try {
		...
		if (profileError) {
			console.error(
				"/api/users/[username]/stats profile error:",
		...
		if (postsError) {
			console.error(
				"/api/users/[username]/stats posts error:",
				postsError.message,
			);
		...
		if (postsDataError) {
			console.error(
				"/api/users/[username]/stats posts data error:",
				postsDataError.message,
			);
		...
		if (postsData && postsData.length > 0) {
			...
			console.log(
				`Fetching views for ${postIds.length} posts for user ${userId}`,
			);
			...
			if (statsError) {
				console.error(
					"/api/users/[username]/stats views error:",
					statsError.message,
				);
			...
			totalViews = (statsData || []).reduce(
				(sum, stat) => sum + (stat.views_total || 0),
				0,
			);
