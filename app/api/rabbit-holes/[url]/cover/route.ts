...
export const POST = withAuth(async (req: NextRequest, { userId }) => {
  try {
    ...
    const extRes = await fetch(`${EXTERNAL_API_URL}/social/v1/profile/cover`, {
      method: "POST",
      body: externalForm,
    });
    if (!extRes.ok) {
      const txt = await extRes.text().catch(() => "External API error");
      logSecureError("rabbit_hole_cover_external_error", new Error(txt), userId);
    ...
    logSecureError("rabbit_hole_cover_error", e, userId);
