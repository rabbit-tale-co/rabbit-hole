	...
	if (body?._action === "delete") {
		...
		if (!token) {
			return Response.json(
				{ error: "JWT token required in Authorization header or body" },
				{ status: 400 },
			);
		}
		...
		console.log(
			`[API] JWT verification result:`,
			jwtResult ? { userId: jwtResult.userId } : "null",
		);
		...
		console.log(
			`[API] Calling deletePost with postId: ${parsed.data}, userId: ${jwtResult.userId}`,
		);
