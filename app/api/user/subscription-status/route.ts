	...
	try {
		...
		if (requestedUserId && requestedUserId !== authenticatedUserId) {
			return NextResponse.json(
				{
					error:
						"Unauthorized: You can only access your own subscription status",
				},
				{ status: 403 },
			);
		}
		...
		return NextResponse.json({
			...
			plan: planName,
			nextBillingDate: activeSubscription.items.data[0].current_period_end
				? new Date(
						activeSubscription.items.data[0].current_period_end * 1000,
					).toISOString()
				: null,
			customerId: stripeCustomer.id,
...
export async function POST() {
	try {
		...
		return NextResponse.json({
			...
			isPremium: isActive,
			subscriptionStatus:
				activeSubscription?.status || latestSubscription?.status || "inactive",
