...
export const POST = withAuth(async (req: NextRequest, { userId: authenticatedUserId }) => {
  try {
    ...
    if (requestedUserId !== authenticatedUserId) {
      logSecureError('avatar_unauthorized_access', new Error('User tried to modify another user\'s avatar'), authenticatedUserId);
      return NextResponse.json({ error: "Unauthorized - You can only modify your own avatar" }, { status: 403 });
    }

    console.log("[avatar] request: userId=%s name=%s type=%s size=%d crop=%d,%d %dx%d", authenticatedUserId, (file as File).name, (file as File).type, (file as File).size, cropX, cropY, cropW, cropH);
    ...
    if (isGif) {
      ...
      if (profileError) {
        logSecureError('avatar_premium_check_error', profileError, authenticatedUserId);
        return NextResponse.json({ error: "Failed to verify premium status" }, { status: 500 });
      }

      if (!profile?.is_premium) {
        logSecureError('avatar_gif_unauthorized', new Error('Non-premium user tried to upload GIF avatar'), authenticatedUserId);
        return NextResponse.json({ error: "GIF avatars are available only for Golden Carrot subscribers" }, { status: 403 });
      }
    }
    ...
    const externalResponse = await fetch(`${EXTERNAL_API_URL}/social/v1/profile/avatar`, {
      method: "POST",
