...
async function syncPremiumStatus(userId: string) {
	try {
		...
		if (
			...
			profile.stripe_customer_id.startsWith("manual_")
		) {
...
export async function POST(req: NextRequest) {
	try {
		...
		if (desiredUsername) {
			const { containsBannableWords, getConfigFromEnv } = await import(
				"@/lib/bannable-words"
			);
		...
		// 1) If exists: ensure accent_color is set and optionally apply desired username
		{
				...
				return Response.json(
					{
						...
						error: exErr.message,
						code: (exErr as { code?: string }).code,
						details: (exErr as { details?: string }).details,
						hint: (exErr as { hint?: string }).hint,
					},
					{ status: 500 },
				);
			}
			if (existing) {
				...
				const candidateOk = desiredUsername
					? USERNAME.safeParse(desiredUsername).success
					: false;
				if (candidateOk) {
					...
					if (base !== existing.username?.toLowerCase()) {
						...
						if (
							...
							existing.display_name.toLowerCase() ===
