	...
	try {
		...
		console.log(
			...
			event.data.object.created
				? new Date((event.data.object.created as number) * 1000).toISOString()
				: "unknown",
		);
...
async function handleSubscriptionUpdate(subscription: Record<string, unknown>) {
	...
	try {
		...
		if (error) {
			...
			console.log(
				`✅ Premium status updated to ${isActive} for user ${userId}`,
			);
		}
	} catch (error) {
...
async function handlePaymentActionRequired(invoice: Record<string, unknown>) {
	...
	try {
		...
		console.log(
			"ℹ️  Keeping premium access - user has grace period to update payment method",
		);
