...
export function BanUserDialog({
	...
	function buildTargetDate(): Date {
		...
		if (mode === "duration")
			return new Date(Date.now() + Math.max(1, durationDays) * 86_400_000);
		...
		<Dialog open={open} onOpenChange={(o) => !busy && onOpenChange(o)}>
			<DialogContent className="sm:max-w-[700px]" aria-busy={busy}>
				<DialogHeader className="space-y-1">
					...
					<DialogDescription>
						Choose how long, add a reason, and confirm.
					</DialogDescription>
							...
							<div className="grid max-w-[420px] grid-cols-2 gap-4">
								...
								<div className="self-end text-xs text-muted-foreground">
									...
									<span className="font-medium">
										{new Date(
											Date.now() + Math.max(1, durationDays) * 86_400_000,
							...
							<div className="space-y-4">
								...
								{isUntilInvalid && (
									<div className="text-xs text-destructive">
										The selected date/time is in the past. Please choose a
										future time.
									</div>
								)}
							</div>
					...
					{mode === "permanent" && (
						<div className="space-y-4">
							<p className="text-sm text-muted-foreground">
								This permanently suspends the account (can be lifted by an
								admin).
							</p>
							<div className="grid gap-4 sm:grid-cols-2">
								<div className="space-y-1.5">
									<Label htmlFor="confirmBan">
										Type <span className="font-mono">&quot;BAN&quot;</span> to
										confirm
									</Label>
									<Input
										id="confirmBan"
								...
								<div className="self-end text-xs text-muted-foreground">
									Effective until:&nbsp;
									<span className="font-medium">{target.toUTCString()}</span>
								</div>
							</div>
						</div>
					)}
