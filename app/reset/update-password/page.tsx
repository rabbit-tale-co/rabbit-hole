	...
	async function onSubmit(e: React.FormEvent) {
		...
		try {
			...
			try {
				...
				console.warn(
					"updateUser timed out or failed, using REST fallback",
					sdkErr,
				);
				...
				if (!access_token) {
					setError("Missing access token for fallback.");
					return;
				}
				if (!url || !anon) {
					setError("Missing Supabase env for fallback.");
					return;
				}
				...
				if (!resp.ok) {
					...
					throw new Error(
						`Auth REST ${resp.status}: ${txt || resp.statusText}`,
					);
				}
			}
